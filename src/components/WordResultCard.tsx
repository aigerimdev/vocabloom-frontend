...
const WordResultCard = ({ data, onSave, onClose, tags, setTags }: WordResultCardProps) => {
    ...
    const handleTagSelect = (tagId: number | null, tagName: string | null) => {
        setSelectedTagId(tagId);
        setSelectedTagName(tagName);
        // optional: clear feedback on change
        setError('');
        setSuccess('');
    };
    ...
    const handleSaveClick = async () => {
        ...
        try {
            ...
            if (savedWord) {
                ...
                setSuccess(
                    selectedTagName
                        ? `Saved to “${selectedTagName}”.`
                        : 'Saved to My Word List.'
                );
            } else {
                setError('Could not save the word. Please try again.');
            }
        } catch (err: any) {
            // if save_word throws friendly message, show it
            setError(err?.message || 'There was an error saving your word.');
        }
    };
    ...
    return (
        <div className="word-card">
            <section className="word-card-header">
                ...
                <h1 className="word-detail-title">{capitalize(data.word)}</h1>
                {data.phonetic && (
                    <p className="word-detail-phonetic">/{data.phonetic}/</p>
                )}
            </section>
            ...
            <div className="word-card-main">
                ...
                {Array.isArray(data.meanings) &&
                    data.meanings.map((meaning, index) => (
                        <div key={index}>
                            <h2 className="word-detail-subtitle">
                                {capitalize(meaning.partOfSpeech)}
                            </h2>
                            <ul className="word-definitions">
                                {meaning.definitions.map((def, defIndex) => (
                                    <li key={defIndex}>
                                        <p className="word-definitions-definition">
                                            {def.definition}
                                        </p>
                                        <p className="word-definitions-example">
